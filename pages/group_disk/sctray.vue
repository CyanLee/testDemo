<template>
	<view class="mcontent">
		<view class="rheader">
			...
			<view class="input-row"><m-input @input="bindChange()" class="m-input" type="text" v-model="ipadd" placeholder="请扫描托盘条码"></m-input></view>
		</view>
				...
				.then(res => {
					...
					if (res.Succeed) {
						if (res.Data.success) {
							...
							this.login(obj);
							var diskobj = {};
							diskobj.fgdtlitem = res.Data.data.fgdtlitem;
							diskobj.baritem = res.Data.data.baritem;
							this.initdata(diskobj);
							uni.navigateTo({
								url: '../group_disk/gdisklist?barcode=' + this.ipadd + '&type=0'
							});
						} else {
							uni.showToast({
								icon: 'none',
