<template>
	<view class="mcontent">
		<view class="rheader">
			<view class="tiptext">入库单</view>
			<view class="input-row"><m-input @input="bindChange()" class="m-input" type="text" v-model="ipadd" placeholder="请扫描入库单"></m-input></view>
		</view>
...
export default {
	...
	methods: {
		...
		bindChange() {
			var data = {
				sf_action: 'TrayPDAGrp_FGI_ValStkInLogNo',
				stkcode: this.stkcode,
				fstkinlogno: this.ipadd,
				slanguage: this.$language,
				usercode: this.userid,
				username: this.username
			};
			uni.showNavigationBarLoading();
			request(this.serverUrl + '/sfapi/TrayPDAGrp_FGI/TrayPDAGrp_FGI_ValStkInLogNo', 'POST', data)
				.then(res => {
					uni.hideNavigationBarLoading();
					if (res.Succeed) {
						if (res.Data.success) {
							...
							this.login(obj);
							uni.navigateTo({
								url: '../group_disk/rktray?barcode=' + this.ipadd
							});
						} else {
							uni.showToast({
								icon: 'none',
