<template>
	<view style="width: 100%;">
		<view class="nofi_view">
			<view>
				<text class="title_text">入库凭单：</text>
		...
		<view class="nofi_view">
			<view>
				<text class="title_text">入库原因：</text>
						...
						<template v-slot:footer>
							<view style="display: flex;align-items: center;">
								<view style="flex: 1;">
									...
									<text style="color: #BD2C00;font-size: 16px;margin-right: 10px;">{{value.fplaninqty}}</text>
									<text class="title_text">{{value.funitname}}</text>
								</view>
								<view style="flex: 1;display: flex;align-items: center;">
									<text class="title_text" style="margin-left: 10px;">实际数：</text>
									<sfCard cardType='1'>
										<input :id="key" class="uni-input" @input="inputChange" placeholder="请输入" v-model="value.factinqty" type="digit" />
										<text class="title_text">{{value.funitname}}</text>
									</sfCard>
								</view>
							</view>
...
<script>
	...
	import {
		StkIn_Other_StkInLogNo,
		StkIn_Other_SaveDatao,
		StkIn_Other_TmpSave,
		StkIn_Other_UnLock
	} from '@/api/api.js'
	import {
	...
	export default {
		...
		data() {
			return {
				...
				fifsupper:0,//控制储位是否可以修改
			}
		},
		methods: {
			getDetail(orderNo) {
				requestHepler({
						url: StkIn_Other_StkInLogNo,
						addStkcode: true,
						data: {
							sorino: orderNo,
						},
					...
					.then(res => {
						...
						this.dataSource = res.stkinitem;
						this.originDataSource = res.stkinitem;
			...
			calculateTotal() {
				...
				for (let i = 0; i < this.dataSource.length; i++) {
					const item = this.dataSource[i];
					if (item.fsel === '1') {
						factinqty = calc.accAdd(factinqty, item.factinqty)
					}
				}
				this.totalMoney = factinqty;
			},
			...
			inputChange(res) {
				...
				if (calc.comAGreaterB(valueTager, itemValue.fplaninqty)) {
					//这里需要特别注意，不用延迟修改会没有效果
					setTimeout(() => {
						this.$set(itemValue, 'factinqty', calc.floorFloat(itemValue.fplaninqty,itemValue.fdecimal))
						this.calculateTotal()
					}, 0)
				} else {
					setTimeout(() => {
						this.$set(itemValue, 'factinqty', valueTager)
						this.calculateTotal()
					}, 0)
				}
			},
