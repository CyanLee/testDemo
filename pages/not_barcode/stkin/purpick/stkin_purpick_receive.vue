	...
	<view class="mcontent">
		<view class="top">
			...
			<view class="input-row" style="border: 1px solid #e5e5e5;text-align: center;">
				<view class="sfrow width40">{{ titledata.fpgno }}</view>
				<view class="sfrow width35">{{ titledata.fpgdate }}</view>
				<view class="sfrow width25">{{ titledata.fcreator }}</view>
			</view>
		</view>
		<view style="margin-top: 102px; height: 100%;">
			<view class="input-row" v-for="(item, index) in dataList" :key="index" @tap="selectOrder(item.fpgno)" :style="{ borderBottom: '0.6px solid #e5e5e5', textAlign: 'center', backgroundColor: index % 2 == 0 ? '#ffffff' : '#f5f5f5' }">
				<view :class="item.fifcolorred ? 'sfrow bodyrow width40 colorred' : 'sfrow bodyrow width40'">{{ item.fpgno }}</view>
				<view class="sfrow bodyrow width35">{{ formatDate(item.fpgdate) }}</view>
	...
	export default {
		...
		data() {
			return {
				...
				loadingType: 'noMore',
				code: '',
				toUrl: '/pages/not_barcode/stkin/purpick/detail?orderNo='
			};
		},
		onBackPress(options) {
		...
		onLoad(option) {
			...
			uni.$on(NOFI_SAVEFINSH_BILL, (orderNo) => {
				for (let i = 0; i < this.dataList.length; i++) {
					if (this.dataList[i].fpgno === orderNo) {
		...
		methods: {
			LoadData(val) {
				var data = {
					sf_action: '/PurPickRc_GetfPGNos',
					stkcode: this.code, //仓库代号
					spage: this.pageIndex,
					pgno: this.ipadd.value
				};
				requestHepler({
						url: PurPickRc_GetfPGNos,
						data: data
					})
					.then(res => {
						if (res) {
							...
							if (res.item != null && res.item.length > 0) {
								...
								if (this.dataList.length == 1 && val && this.ipadd != '') {
									uni.navigateTo({
										url: this.toUrl + res.item[0].fpgno
									});
									return;
								}
