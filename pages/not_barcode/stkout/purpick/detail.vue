<template>
	<view style="width: 100%;">
		<view class="nofi_view">
			<view>
				<text class="title_text">退货单：</text>
				<text class="des_text" :style="{ color: orderNoStyle(mst.fifcolorred) }">{{ mst.frtnno }}</text>
			</view>
		...
		<view style="width: 100%; height: auto;"><uniSearchBar radius="5" placeholder="品名/品号" clearButton="auto" cancelText="" @confirm="searchResponse" /></view>
				...
				<view v-for="(value, key) in dataSource" :key="key">
					<uniCard :is-shadow="true" :show-arrow="false" note="true">
						...
						<view style="display: flex;margin-top: 8px;">
							...
							<text class="title_text">{{ value.fpurno }}</text>
						</view>
						...
						<view
							...
							style="display: flex;align-items: center;margin-top: 8px;width: 100%;"
							@tap="value.fiflvlmng === '2' ? '' : conditionsSelect(key, 0)"
						>
							<text class="des_text">等级：</text>
							<sfCard style="width: 100%;" :title="value.flvlname" :disable="value.fiflvlmng === '2'"></sfCard>
						</view>

						<view v-if="fIfUsePlace == 1" style="display: flex;align-items: center;margin-top: 8px;width: 100%;" @tap="fifsupper == 0 ? '' : conditionsSelect(key, 1)">
							<text class="des_text">储位：</text>
							<sfCard style="width: 100%;" :title="value.fplacecode" :disable="fifsupper == 0"></sfCard>
						</view>

						<view
							v-if="value.fiflotmng !== '0'"
							style="display: flex;align-items: center;margin-top: 8px;width: 100%;"
							@tap="value.fiflotmng === '2' ? '' : conditionsSelect(key, 2)"
						>
							<text class="des_text">批号：</text>
							<sfCard style="width: 100%;" :title="value.flotno" :disable="value.fiflotmng === '2'"></sfCard>
						</view>

						<template v-slot:footer>
							<view style="display: flex;align-items: center;">
								<view style="flex: 1;">
									<text class="title_text">退货数：</text>
									<text style="color: #BD2C00;font-size: 16px;margin-right: 10px;">{{ value.frtnqtyofinvunit }}</text>
									<text class="title_text">{{ value.finvunitname }}</text>
								</view>
								<view style="flex: 1;display: flex;align-items: center;">
									<text class="title_text" style="margin-left: 10px;">实际数：</text>
									<sfCard cardType="1">
										<input :id="key" class="uni-input" @input="inputChange" placeholder="请输入" v-model="value.factqtyofinvunit" type="digit" />
										<text class="title_text">{{ value.finvunitname }}</text>
									</sfCard>
								</view>
							</view>
...
import { PurRtn_GetRtnItem, PurRtn_Save, PurRtn_SaveTemp, PurRtn_Unlock } from '@/api/api.js';
...
export default {
	...
	methods: {
		...
		//计算合计数
		calculateTotal() {
			let factqtyofinvunit = 0;
			for (let i = 0; i < this.dataSource.length; i++) {
				const item = this.dataSource[i];
				if (item.fsel === '1') {
					factqtyofinvunit = calc.accAdd(factqtyofinvunit, item.factqtyofinvunit);
				}
			}
			this.totalMoney = factqtyofinvunit;
		},
		...
		inputChange(res) {
			...
			if (calc.comAGreaterB(valueTager, itemValue.frtnqtyofinvunit)) {
				//这里需要特别注意，不用延迟修改会没有效果
				setTimeout(() => {
					this.$set(itemValue, 'factqtyofinvunit', calc.floorFloat(itemValue.frtnqtyofinvunit, itemValue.finvdecimal));
					this.calculateTotal();
				}, 0);
			} else {
				setTimeout(() => {
					this.$set(itemValue, 'factqtyofinvunit', valueTager);
					this.calculateTotal();
				}, 0);
			}
		},
		...
		//退出的时候需要解锁单据
		requestUnLock() {
			// this.mst.frtnno = 'W0520200117003'
			if (!this.mst.frtnno) return;
			requestHepler({
				url: PurRtn_Unlock,
				addStkcode: true,
				data: {
					pgno: this.mst.frtnno
				}
			}).then(res => {
				uni.navigateBack({});
			});
	...
	onBackPress(e) {
		...
		if (e.from === 'backbutton') {
			//点击物理按钮和左上角按钮才调用
			if (!this.mst.frtnno) return false;
			//如果单据被锁了就直接返回
			else {
				this.requestUnLock();
