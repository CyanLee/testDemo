	...
	<view class="mcontent">
		<view class="top">
			...
			<view class="input-row" style="border: 1px solid #e5e5e5;text-align: center;">
				<view class="sfrow width40">{{ titledata.forino }}</view>
				<view class="sfrow width35">{{ titledata.fcdate }}</view>
				<view class="sfrow width25">{{ titledata.femp }}</view>
			</view>
		</view>
		<view style="margin-top: 102px; height: 100%;">
			<view class="input-row" v-for="(item, index) in dataList" :key="index" @tap="selectOrder(item.forino)" :style="{ borderBottom: '0.6px solid #e5e5e5', textAlign: 'center', backgroundColor: index % 2 == 0 ? '#ffffff' : '#f5f5f5' }">
				<view :class="item.fifcolorred ? 'sfrow bodyrow width40 colorred' : 'sfrow bodyrow width40'">{{ item.forino }}</view>
				<view class="sfrow bodyrow width35">{{ formatDate(item.fcdate) }}</view>
				<view class="sfrow bodyrow width25">{{ item.femp }}</view>
			</view>
		</view>
		<view>
	...
	export default {
		...
		data() {
			return {
				...
				loadingType: 'noMore',
				code: '',
				toUrl: '/pages/not_barcode/stkout/other/detail?orderNo='
			};
		},
		onBackPress(options) {
		...
		onLoad(option) {
			...
			uni.$on(NOFI_SAVEFINSH_BILL, (orderNo) => {
				for (let i = 0; i < this.dataList.length; i++) {
					if (this.dataList[i].forino === orderNo) {
		...
		methods: {
			LoadData(val) {
				var data = {
					sf_action: 'GetStkOutNoList_PDA',
					stkcode: this.code, //仓库代号
					spage: this.pageIndex,
					sorino: this.ipadd.value
				};
				requestHepler({
						url: GetStkOutNoList_PDA,
						...
						if (res) {
							...
							if (res.item != null && res.item.length > 0) {
								...
								if (this.dataList.length == 1 && val && this.ipadd != '') {
									uni.navigateTo({
										url: this.toUrl + res.item[0].forino
									});
									return;
								}
